...
const PLATFORMS: { id: PlatformType; label: string; icon: string; color: string; gradient: string }[] = [
  { id: 'x', label: 'X (Twitter)', icon: '𝕏', color: '#f4f4f4', gradient: 'rgba(236,236,236,0.12)' },
...
const BrandingManager: React.FC<BrandingManagerProps> = ({ storagePrefix }) => {
  ...
  const growthBadge = (current: number, previous: number | undefined) => {
    ...
    return (
      <span className={`flex items-center gap-0.5 text-[11px] font-medium ${diff > 0 ? 'text-[#6dd49a]' : 'text-[#d46d6d]'}`}>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Total Audience */}
        <div className="bg-[#0a0a0a] p-5 rounded-none-none">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[#5a5a5a] text-xs font-medium uppercase tracking-wider">Total Audience</p>
            <div className="p-1.5 rounded-none-none" style={{ background: 'rgba(16,185,129,0.1)' }}>
              <Users size={14} className="text-[#6dd49a]" />
            </div>
          </div>
          <h3 className="text-2xl font-bold text-[#f4f4f4] mb-1">{fmt(totalFollowers)}</h3>
          <p className="text-[11px] text-[#5a5a5a]">Across all platforms</p>
        </div>

        {/* Total Views */}
        <div className="bg-[#0a0a0a] p-5 rounded-none-none">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[#5a5a5a] text-xs font-medium uppercase tracking-wider">Total Views</p>
            <div className="p-1.5 rounded-none-none" style={{ background: 'rgba(16,185,129,0.1)' }}>
              <Eye size={14} className="text-[#6dd49a]" />
            </div>
          </div>
          <h3 className="text-2xl font-bold text-[#f4f4f4] mb-1">{fmt(totalViews)}</h3>
          <p className="text-[11px] text-[#5a5a5a]">Lifetime impressions</p>
        </div>

        {/* Total Posts */}
        <div className="bg-[#0a0a0a] p-5 rounded-none-none">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[#5a5a5a] text-xs font-medium uppercase tracking-wider">Total Posts</p>
            <div className="p-1.5 rounded-none-none" style={{ background: 'rgba(255,255,255,0.04)' }}>
              <FileText size={14} className="text-[#909090]" />
            </div>
          </div>
          <h3 className="text-2xl font-bold text-[#f4f4f4] mb-1">
        ...
        <div className="bg-[#0a0a0a] p-5 rounded-none-none">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[#5a5a5a] text-xs font-medium uppercase tracking-wider">Platforms</p>
            <div className="p-1.5 rounded-none-none" style={{ background: 'rgba(255,255,255,0.04)' }}>
              <TrendingUp size={14} className="text-[#909090]" />
            </div>
          </div>
          <h3 className="text-2xl font-bold text-[#f4f4f4] mb-1">{accounts.filter(a => a.connected).length}/3</h3>
          <p className="text-[11px] text-[#5a5a5a]">Connected</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {PLATFORMS.map(p => {
          ...
          return (
            <div key={p.id} className="bg-[#0a0a0a] rounded-none-none p-5 border border-[#1a1a1a] hover:border-[#242424] transition-colors">
              {/* Platform Header */}
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-none-none flex items-center justify-center text-lg" style={{ background: p.gradient }}>
                    {p.icon}
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-[#f4f4f4]">{p.label}</h3>
                    {account ? (
                      <p className="text-[11px] text-[#5a5a5a]">@{account.username}</p>
                    ) : (
                      <p className="text-[11px] text-[#5a5a5a]">Not connected</p>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  {account && (
                    <button
                      ...
                      className="p-1.5 rounded-none-none text-[#5a5a5a] hover:text-[#f4f4f4] hover:bg-[rgba(255,255,255,0.05)] transition-colors"
                      title="Add snapshot"
                    >
                      <Plus size={14} />
                    </button>
              ...
              {/* Username Editor */}
              {isEditing && (
                <div className="flex items-center gap-2 mb-4 bg-[#060606] rounded-none-none p-2.5 border border-[#1a1a1a]">
                  <span className="text-[#5a5a5a] text-sm">@</span>
                  <input
                    value={usernameInput}
                    onChange={e => setUsernameInput(e.target.value)}
                    placeholder="username"
                    className="flex-1 bg-transparent text-sm text-[#f4f4f4] outline-none placeholder-[#444]"
                    autoFocus
                  ...
                  <button
                    ...
                    className="p-1 rounded-none-none text-[#6dd49a] hover:bg-[rgba(16,185,129,0.1)]"
                  >
                    <Check size={14} />
                  </button>
                  <button onClick={() => setEditingAccount(null)} className="p-1 rounded-none-none text-[#5a5a5a] hover:text-[#909090]">
                    <X size={14} />
                  </button>
                  {account && (
                    <button onClick={() => { removeAccount(p.id); setEditingAccount(null); }} className="p-1 rounded-none-none text-[#d46d6d] hover:bg-[rgba(239,68,68,0.1)]" title="Disconnect">
                      <Trash2 size={14} />
                    </button>
                  )}
                </div>
              ...
              {latest ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] text-[#5a5a5a] uppercase tracking-wider">{followerLabel}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-bold text-[#f4f4f4]">{fmt(followerCount)}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-2" style={{ borderTop: '1px solid #252525' }}>
                    <div>
                      <p className="text-[10px] text-[#5a5a5a] uppercase tracking-wider mb-0.5">Views</p>
                      <p className="text-sm font-semibold text-[#f4f4f4]">{fmt(latest.total_views)}</p>
                      {growthBadge(latest.total_views, prev?.total_views)}
                    </div>
                    <div>
                      <p className="text-[10px] text-[#5a5a5a] uppercase tracking-wider mb-0.5">Posts</p>
                      <p className="text-sm font-semibold text-[#f4f4f4]">{fmt(latest.total_posts)}</p>
                      {growthBadge(latest.total_posts, prev?.total_posts)}
                    </div>
                    {p.id !== 'youtube' && (
                      <div>
                        <p className="text-[10px] text-[#5a5a5a] uppercase tracking-wider mb-0.5">Following</p>
                        <p className="text-sm font-semibold text-[#f4f4f4]">{fmt(latest.following)}</p>
                      </div>
                    )}
                    {p.id === 'youtube' && (
                      <div>
                        <p className="text-[10px] text-[#5a5a5a] uppercase tracking-wider mb-0.5">Videos</p>
                        <p className="text-sm font-semibold text-[#f4f4f4]">{fmt(latest.total_posts)}</p>
                      </div>
                    )}
                  </div>
                ...
                <div className="text-center py-6">
                  <p className="text-[#3a3a3a] text-sm mb-2">No data yet</p>
                  {account ? (
                    <button
                      onClick={() => setAddSnapshotModal(p.id)}
                      className="text-xs text-[#6dd49a] hover:text-[#6dd49a] font-medium"
                    >
                      + Add first snapshot
                    </button>
                  ) : (
                    <p className="text-[11px] text-[#3a3a3a]">Connect your account first</p>
                  )}
                </div>
              )}
            </div>
      ...
      <div className="bg-[#0a0a0a] rounded-none-none p-6">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div>
            <h2 className="text-sm font-bold text-[#f4f4f4]">Audience Growth</h2>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex bg-[#060606] rounded-none-none p-0.5 border border-[#1a1a1a]">
              {(['followers', 'views'] as const).map(m => (
                <button key={m} onClick={() => setChartMetric(m)}
                  className={`px-3 py-1.5 rounded-none-none text-xs font-medium transition-colors ${chartMetric === m ? 'bg-[#0d0d0d] text-[#f4f4f4]' : 'text-[#5a5a5a] hover:text-[#909090]'}`}>
            ...
            <div className="flex bg-[#060606] rounded-none-none p-0.5 border border-[#1a1a1a]">
              <button onClick={() => setChartPlatform('all')}
                className={`px-3 py-1.5 rounded-none-none text-xs font-medium transition-colors ${chartPlatform === 'all' ? 'bg-[#0d0d0d] text-[#f4f4f4]' : 'text-[#5a5a5a] hover:text-[#909090]'}`}>
                All
              </button>
              {PLATFORMS.map(p => (
                <button key={p.id} onClick={() => setChartPlatform(p.id)}
                  className={`px-2.5 py-1.5 rounded-none-none text-xs transition-colors ${chartPlatform === p.id ? 'bg-[#0d0d0d]' : 'text-[#5a5a5a] hover:text-[#909090]'}`}>
                  {p.icon}
                </button>
              ))}
            </div>
            ...
            <div className="flex bg-[#060606] rounded-none-none p-0.5 border border-[#1a1a1a]">
              {(['week', 'month', '3months', 'year', 'all'] as const).map(v => (
                <button key={v} onClick={() => setChartView(v)}
                  className={`px-3 py-1.5 rounded-none-none text-xs font-medium transition-colors ${chartView === v ? 'bg-[#0d0d0d] text-[#f4f4f4]' : 'text-[#5a5a5a] hover:text-[#909090]'}`}>
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                />
                <Tooltip
                  contentStyle={{ background: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: 0, fontSize: 12, color: '#f4f4f4' }}
            ...
            <div className="h-full flex flex-col items-center justify-center text-[#3a3a3a]">
              <TrendingUp size={32} className="mb-3 text-[#333]" />
              <p className="text-sm font-medium mb-1">No data yet</p>
              <p className="text-xs text-[#3a3a3a]">Add snapshots to your platforms to see growth over time</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-[#0a0a0a] rounded-none-none p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-sm font-bold text-[#f4f4f4]">Snapshot History</h2>
            <p className="text-[11px] text-[#5a5a5a] mt-0.5">Daily metrics log — add a snapshot whenever you check your stats</p>
          </div>
          <span className="text-[11px] text-[#5a5a5a]">{snapshots.length} snapshots</span>
        </div>

        {snapshots.length === 0 ? (
          <p className="text-center text-[#3a3a3a] text-sm py-8">No snapshots yet. Connect a platform and add your first snapshot.</p>
        ) : (
          <div className="space-y-1">
            {/* Header */}
            <div className="grid grid-cols-12 gap-3 px-3 py-2 text-[10px] font-medium uppercase tracking-wider text-[#5a5a5a]">
            ...
            {[...snapshots].reverse().map(snap => {
              ...
              return (
                <div key={snap.id} className="grid grid-cols-12 gap-3 items-center px-3 py-2.5 rounded-none-none hover:bg-[rgba(255,255,255,0.03)] transition-colors group">
                  <div className="col-span-2 text-[12px] text-[#909090]">
                  ...
                  <div className="col-span-2 flex items-center gap-2">
                    ...
                    <span className="text-[12px] text-[#f4f4f4]">{pConfig?.label}</span>
                  </div>
                  <div className="col-span-2 text-right text-[13px] font-semibold text-[#f4f4f4]">
                    {fmt(snap.platform === 'youtube' ? snap.subscribers : snap.followers)}
                  </div>
                  <div className="col-span-2 text-right text-[12px] text-[#909090]">{fmt(snap.total_views)}</div>
                  <div className="col-span-2 text-right text-[12px] text-[#909090]">{fmt(snap.total_posts)}</div>
                  <div className="col-span-2 text-right flex items-center justify-end gap-2">
                    <span className="text-[12px] text-[#909090]">{snap.platform !== 'youtube' ? fmt(snap.following) : '—'}</span>
                    <button
                      onClick={() => deleteSnapshot(snap.id)}
                      className="p-1 rounded-none text-[#333] hover:text-[#d46d6d] opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      {addSnapshotModal && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-none-none w-full max-w-md overflow-hidden">
            <div className="p-5 border-b border-[#1a1a1a]">
              <h3 className="text-sm font-semibold text-[#f4f4f4]">
                Add Snapshot — {PLATFORMS.find(p => p.id === addSnapshotModal)?.label}
              </h3>
              <p className="text-xs text-[#5a5a5a] mt-1">Enter your current stats from the platform</p>
            </div>
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1 bg-[#060606] border border-[#1a1a1a] rounded-none-none px-3 py-2.5 text-sm text-[#f4f4f4] focus:outline-none focus:ring-1 focus:ring-[#555]"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-[#5a5a5a] font-medium">
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="w-full mt-1 bg-[#060606] border border-[#1a1a1a] rounded-none-none px-3 py-2.5 text-sm text-[#f4f4f4] focus:outline-none focus:ring-1 focus:ring-[#555] placeholder-[#444]"
                  />
                </div>
                {addSnapshotModal !== 'youtube' && (
                  <div>
                    <label className="text-[10px] uppercase tracking-wider text-[#5a5a5a] font-medium">Following</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="text-[10px] uppercase tracking-wider text-[#5a5a5a] font-medium">Total Views</label>
                  <input
                    type="number"
            ...
            <div className="flex justify-end gap-3 p-4 border-t border-[#1a1a1a]">
              <button
                ...
                className="px-4 py-2 bg-[#0d0d0d] hover:bg-[#333] text-[#f4f4f4] rounded-none-none text-sm font-medium transition-colors"
              >
                Cancel
              </button>
              <button
                ...
                disabled={saving}
                className="px-4 py-2 bg-white hover:bg-[#e5e5e5] text-[#212121] rounded-none-none text-sm font-medium transition-colors disabled:opacity-50"
              >
